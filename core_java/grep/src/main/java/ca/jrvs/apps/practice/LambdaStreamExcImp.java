...
public class LambdaStreamExcImp implements LambdaStreamExc {
    ...
    public Stream<String> filter(Stream<String> stringStream, String pattern) {
        ...
        return stringStream.filter(s -> !regexPattern.matcher(s).matches());
    }
    ...
    public IntStream createIntStream(int start, int end) {
        // rangedClosed - Returns a sequential ordered IntStream from startInclusive (inclusive) to endInclusive (inclusive) by an incremental step of 1.
    ...
    public void printOdd(IntStream intStream, Consumer<String> printer) {
        intStream.filter(n -> n % 2 != 0).forEach(n -> printer.accept(String.valueOf(n)));
    }
    ...
    public Stream<Integer> flatNestedInt(Stream<List<Integer>> ints) {
        return ints.flatMap(n -> n.stream()).map(n -> n * n);
    }
}
